...
function ResetPass(){
 ...
 if (mysqli_connect_errno())
	...
	$sql= 'UPDATE staff SET `password` = "'.$password.'", confirmPassword = "'.$confirmPassword.'" WHERE email = "'.$email.'"';
	echo $sql;	
	$qry = mysqli_query($con,$sql);
...
print_r($_POST);
 ...
 if (mysqli_connect_errno())
  ...
  $confirmPassword=$_POST['confirmPassword'];
	$sql= 'UPDATE staff SET `password` = "'.$newPassword.'", confirmPassword="'.$confirmPassword.'" WHERE email = "'.$email.'"';
	echo $sql;	
	$qry = mysqli_query($con,$sql);
